...
- Naming: crates/modules/functions `snake_case`; types `CamelCase`; constants `SCREAMING_SNAKE_CASE`.
- Gateway features follow 3-layer pattern: `handler.rs` (HTTP), `service.rs` (business), `repository.rs` (data).
...
- PRs must include: clear description, linked issues, reproduction steps, and relevant screenshots/logs (e.g., curl output).
- Requirements: CI green (tests, fmt, clippy, prettier, audit) and `./scripts/check-security.sh` passes.

## Security & Configuration Tips
- Copy `.env.example` → `.env` for local dev; never commit `.env`.
- Do not enable `AUTH_DEVELOPMENT_MODE` in committed code or CI. Local-only usage is allowed but must not be pushed.
